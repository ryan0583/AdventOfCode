...
public class Day4 {
    ...
    private static List<String> input = Arrays.asList("[1518-01-19 00:03] Guard #1021 begins shift",
    ...
    public static void partOne() {
        ...
        HashMap<Integer, Integer> guardNumberToNumMinutes = new HashMap<>();

        HashMap<Integer, HashMap<Integer, Integer>> guardNumberToMinuteCount = new HashMap<>();

        int currentGuardNumber = -1;
        int fallsAsleep = 0;

        for (String str : input) {
            int parsedGuardNumber = parseGuardNumber(str);
            if (parsedGuardNumber > -1) {
                currentGuardNumber = parsedGuardNumber;
                fallsAsleep = 0;
                if (!guardNumberToNumMinutes.containsKey(parsedGuardNumber)) {
                    guardNumberToNumMinutes.put(parsedGuardNumber, 0);
                }
                if (!guardNumberToMinuteCount.containsKey(parsedGuardNumber)) {
                    guardNumberToMinuteCount.put(parsedGuardNumber, new HashMap<>());
                }
            } else {
                int parsedFallsAsleepMinute = parseFallsAsleepMinute(str);
                if (parsedFallsAsleepMinute > -1) {
                    fallsAsleep = parsedFallsAsleepMinute;
                } else {
                    int parsedWakesUpMinute = parseWakesUpMinute(str);
                    if (parsedWakesUpMinute > -1) {
                        performWakeUpActions(guardNumberToNumMinutes, guardNumberToMinuteCount, currentGuardNumber, fallsAsleep, parsedWakesUpMinute);
                    }
                }
            }
        }

        int sleepyGuardNumber = findKeyWithMaxValue(guardNumberToNumMinutes);

        HashMap<Integer, Integer> minutesToCount = guardNumberToMinuteCount.get(sleepyGuardNumber);
        int sleepyMinute = findKeyWithMaxValue(minutesToCount);

        int output = sleepyGuardNumber * sleepyMinute;

        System.out.println(output);

        partTwo(guardNumberToMinuteCount);
    }

    private static void partTwo(HashMap<Integer, HashMap<Integer, Integer>> guardNumberToMinuteCount) {
        System.out.println("Day Four, Part Two:");

        int maxFrequency = 0;
        int mostFreqMinute = -1;
        int mostFreqGuardNum = -1;

        for (int guardNumber : guardNumberToMinuteCount.keySet()) {
            HashMap<Integer, Integer> minutesToCount = guardNumberToMinuteCount.get(guardNumber);
            for (int minute : minutesToCount.keySet()) {
                int count = minutesToCount.get(minute);
                if (count > maxFrequency) {
                    maxFrequency = count;
                    mostFreqMinute = minute;
                    mostFreqGuardNum = guardNumber;
                }
            }
        }

        int output = mostFreqGuardNum * mostFreqMinute;

        System.out.println(output);
    }

    private static int findKeyWithMaxValue(HashMap<Integer, Integer> guardNumberToNumMinutes) {
        int maxValue = 0;
        int keyWithMaxValue = -1;

        for (int guardNumber : guardNumberToNumMinutes.keySet()) {
            int minutes = guardNumberToNumMinutes.get(guardNumber);
            if (minutes > maxValue) {
                maxValue = minutes;
                keyWithMaxValue = guardNumber;
            }
        }
        return keyWithMaxValue;
    }

    private static void performWakeUpActions(HashMap<Integer, Integer> guardNumberToNumMinutes,
                                             HashMap<Integer, HashMap<Integer, Integer>> guardNumberToMinuteCount,
                                             int currentGuardNumber,
                                             int fallsAsleep,
                                             int parsedWakesUpMinute) {
        int minutesAsleep = parsedWakesUpMinute - fallsAsleep;
        int currentMinutes = guardNumberToNumMinutes.get(currentGuardNumber);
        currentMinutes += minutesAsleep;
        guardNumberToNumMinutes.put(currentGuardNumber, currentMinutes);

        HashMap<Integer, Integer> minutesToCount = guardNumberToMinuteCount.get(currentGuardNumber);
        for (int i = fallsAsleep; i < parsedWakesUpMinute; i++) {
            if (minutesToCount.containsKey(i)) {
                int existingCount = minutesToCount.get(i);
                existingCount++;
                minutesToCount.put(i, existingCount);
            } else {
                minutesToCount.put(i, 1);
            }
        }
    }

    private static int parseGuardNumber(String str) {
        int hashIndex = str.indexOf('#');
        int beginsIndex = str.indexOf("begins");
        if (hashIndex > -1 && beginsIndex > -1) {
            return Integer.parseInt(str.substring(hashIndex + 1, beginsIndex - 1));
        }
        return -1;
    }

    private static int parseFallsAsleepMinute(String str) {
    ...
    private static int parseWakesUpMinute(String str) {
